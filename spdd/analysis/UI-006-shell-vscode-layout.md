...
# Analyse — UI-006 (delta) — Shell VS Code-style + theming Kiro
...
`ActivityBar`, `SidebarPanel` (collapsable), `VSCode-layout`, `Kiro-theme`,
...
- **`frontend/src/styles/globals.css`** — déjà structuré en CSS
  ...
  valeurs `.dark` pour la palette Kiro et force `dark` sur l'élément
...
- **`<ProjectPicker />` (UI-001b)** — inchangé. S'affiche quand
  `projectDir === ''`. Bénéficie automatiquement du theming Kiro
...
- **CSS variables Kiro** dans `globals.css` (modif) :
  ```css
  .dark {
...
| `frontend/src/styles/globals.css` | moyen | overrides palette Kiro sur `.dark` |
...
- **Theming Kiro qui casse certaines lisibilités** *(prob. moyenne,
