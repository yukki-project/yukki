...
# Canvas REASONS — Shell VS Code-style + theming Kiro-inspired
...
loin des conventions IDE modernes (VS Code, Cursor, Kiro, Zed) que
le public cible côtoie. UI-006 le refond en **double sidebar
VS Code-style** (activity bar fixe + panel collapsable) avec un
theming **Kiro-inspired** (sombre + accent violet), sans casser
...
- [ ] `frontend/src/styles/globals.css` modifié : valeurs `.dark`
      remplacées par la palette Kiro (fond `~#1A1A1F`, surface
...
- **I6** — Theming : la palette Kiro est définie **uniquement**
  dans `globals.css` `.dark`. Aucun composant ne hardcode une
  couleur Kiro en Tailwind ou en style inline ; tout passe par
...
> et l'aligner sur les conventions IDE (VS Code / Cursor / Kiro)
...
> Kiro-inspired sur `.dark` (forcé `<html class="dark">`), et la
...
> libère 240px), identité visuelle distinctive (palette Kiro), et
...
- **OQ7 → A** : palette Kiro s'applique aussi au Dialog (CSS vars).
...
- **Garder `<Sidebar />` UI-001b et juste appliquer le theming Kiro**
  — ne livre pas la double sidebar VS Code-style demandée par la
  story.
- **Activity bar à droite** — non conforme à VS Code/Cursor/Zed/Kiro.
...
| `frontend/src/styles/globals.css` | modif | overrides palette Kiro sur `.dark` |
...
### O1 — Theming Kiro dans `globals.css` + `<html class="dark">`
...
- **CSS variables shadcn uniquement** : aucun composant ne hardcode
  un hex color Kiro. Tout passe par `bg-background`, `text-foreground`,
...
- **Theming hardcodé interdit**
  - Aucun composant n'écrit de hex color (`#1A1A1F`, `#8B5CF6`,
    ...
    shadcn ou les vars CSS. Si une couleur Kiro doit être
