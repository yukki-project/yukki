...
sibling-stories:
  ...
  - META-003-yukki-tests-skill
depends-on: []
---

# Renommer `spdd/` en `.yukki/` et `/yukki-*` en `/yukki-*`
...
ses commandes via les slash `/yukki-*`. Ces deux conventions portent le nom
...
- Renommage des skills slash commands `/yukki-* → /yukki-*` :
  - fichiers `.claude/commands/yukki-*.md` → `.claude/commands/yukki-*.md`
    (8 fichiers)
  - dossiers `.github/skills/yukki-*/` → `.github/skills/yukki-*/`
    (8 dossiers, contenu `SKILL.md` synchronisé)
- Mise à jour des **références internes** entre skills (les skills se
  citent mutuellement, ex. "puis `/yukki-analysis`" → "puis `/yukki-analysis`")
...
- Mise à jour des chaînes affichées dans le frontend :
  ...
  - [`frontend/src/components/workflow/CreateNextStageModal.tsx`](../../frontend/src/components/workflow/CreateNextStageModal.tsx)
    ...
    (mention des commands `/yukki-*` → `/yukki-*`)
- Mise à jour des liens et chemins dans le contenu des skills (chemins
  `.yukki/templates/...`, `.yukki/methodology/...` → `.yukki/...`) — miroir
...
- Compatibilité ascendante "lit aussi `spdd/` si présent" ou "fallback
  vers `/yukki-*`" — projet en phase dev, on bascule franchement, un seul
  layout supporté.
...
- **Given** les frontmatter `story: .yukki/stories/<id>.md` et
  `analysis: .yukki/analysis/<id>.md` actuellement présents dans canvas
  et analyses
...
- **Given** les skills actuellement nommés `.claude/commands/yukki-*.md`
  et `.github/skills/yukki-*/SKILL.md`
...
- **Then** les libellés affichés sont `/yukki-story`, `/yukki-analysis`,
  ...
  occurrence `/yukki-*` n'apparaît dans le rendu
...
- ✅ **Préfixe `.`** confirmé. Le `.` est délibéré : il permet de
  ...
  l'ensemble.
- ✅ **Renommage des slash commands `/yukki-* → /yukki-*`** intégré au
...
| **Interfaces** (CLI / UI / API) | Non scindable | Dossier + commands forment un même bloc de convention ; les séparer crée un état mixte (dossier renommé mais commands toujours `/yukki-*`, ou inverse) |
...
ou où le dossier est renommé mais les commands restent `/yukki-*`. Trace
...
| Skills (rename + contenu) | `.claude/commands/yukki-*.md` → `yukki-*.md` (8 fichiers), `.github/skills/yukki-*/` → `yukki-*/` (8 dossiers) |
