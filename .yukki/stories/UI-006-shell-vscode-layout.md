...
title: Shell VS Code-style — activity bar + sidebar collapsable + theming yukki dark
status: synced
created: 2026-05-02
updated: 2026-05-06
...
# Shell VS Code-style — activity bar + sidebar collapsable + theming yukki dark
...
des conventions des outils desktop modernes (VS Code, Cursor, Zed)
que l'utilisateur cible côtoie au quotidien.
...
inspiration **dark theme moderne** (sombre, accent violet,
typographie compacte).

Référence visuelle fournie par l'utilisateur : capture d'un IDE en spec
mode (implementation plan view) — fond très sombre, bordures fines,
...
- **Identité visuelle cohérente** : palette dark accent violet aligne
  yukki sur Cursor/Zed plutôt que sur le default shadcn slate
...
### AC5 — Theming yukki dark sur fond sombre
...
- [x] **OQ7 → A** : palette yukki dark **s'applique aussi** au
...
- **Filiation** : story de polish post-V1. Pas une fille de UI-001.
  ...
  leur intégration.
- **Référence visuelle** fournie par l'utilisateur : screenshot d'un
  IDE en spec mode — fond très sombre, accent violet, typographie
  compacte, panneau droit "New Session" séparé. Capture conservée
  hors-repo (référence design uniquement).
