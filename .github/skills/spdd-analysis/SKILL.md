...
Utiliser `#codebase` et `#search` pour explorer chaque module listé dans le front-matter de la story :

| Module | Où chercher | Exemples de patterns |
|---|---|---|
| `backend` | `backend/src/main/java/**`, `backend/src/main/resources/**` | `*Resource.java`, `*Service.java` |
| `controller` | `controller/src/main/java/**` | `*Reconciler.java`, `*Controller.java` |
| `common` | `common/src/main/java/**` | DTOs, modèles CRD |
| `frontend` | `frontend/projects/**/*.ts`, `*.html`, `*.scss` | services, reducers, effects, composants |
| `extensions/<nom>` | `extensions/<nom>/**` | `manifest.yaml`, `index.js`, sources |
| `helm` | `helm/`, `examples/overlays/**` | `values.yaml`, templates |
| `docs` | `docs/modules/**/*.adoc` | pages Antora |

**Règles importantes** :
- **Scan ciblé, pas exhaustif** — l'objectif est de poser un diagnostic, pas de tout lire.
- Si un mot-clé renvoie plus de ~50 résultats, raffiner avec un autre mot-clé combiné, ne pas tout lire.
- Préférer plusieurs requêtes `#search` précises à une lecture intégrale de fichiers.
